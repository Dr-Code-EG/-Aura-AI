...
//
// The host application (PyQt) calls window.drcodeSendScreenshot(b64, question)
...
(function () {
  if (window.__drcodeChatGPTBridgeInstalled) {
    return;
  }
  window.__drcodeChatGPTBridgeInstalled = true;
  ...
  window.drcodeSendScreenshot = async function (b64, question) {
    if (isLoginUrl(location.href)) {
      throw new Error("DRCODE_LOGIN_REQUIRED");
    }
    if (isLoggedOut()) {
      throw new Error("DRCODE_LOGIN_REQUIRED");
    }
    ...
    const final = await waitForResponse((partial) => {
      if (window.drcodeOnPartial) {
        try { window.drcodeOnPartial(partial); } catch (_) {}
      }
    }, 120000);
    return final;
  };

  window.drcodeIsReady = function () {
